...
from app.core.llm.common import extract_think_chain, extract_answer
from app.core.llm.ollama import OllamaAPI
...
class LLMClient:
    ...
    async def chat_stream(self, prompt: Union[str, List[dict]], options: dict = None):
        """
        流式生成对话响应
        Args:
            prompt: 用户输入的提示词或对话历史
        ...
        provider = self.provider.lower()
        if provider == 'ollama':
            return await self._chat_ollama_stream(messages, options)
        elif provider in ['openai', 'siliconflow', 'deepseek']:
            return await self._chat_openai_stream(messages, options)
        elif provider == 'zhipu':
            return await self._chat_zhipu_stream(messages, options)
        else:
            return await self._chat_openai_stream(messages, options)

    async def get_response(self, prompt: Union[str, List[dict]], options: dict = None) -> str:
        """获取响应文本"""
        llm_res = await self.chat(prompt, options)
        return (llm_res.get('choices', [{}])[0].get('message', {}).get('content') or
                llm_res.get('response') or
                '')

    async def get_response_with_cot(self, prompt: Union[str, List[dict]], options: dict = None) -> dict[str, str]:
        """获取带有思维链的响应"""
        llm_res = await self.chat(prompt, options)
        answer = (llm_res.get('choices', [{}])[0].get('message', {}).get('content') or
                  llm_res.get('response') or
                  '')
        cot = ''

        if answer.startswith('<think>') or answer.startswith('<thinking>'):
            cot = extract_think_chain(answer)
            answer = extract_answer(answer)
        else:
            cot = (llm_res.get('choices', [{}])[0].get('message', {}).get('reasoning_content') or
                   llm_res.get('choices', [{}])[0].get('message', {}).get('reasoning') or
                   '')
